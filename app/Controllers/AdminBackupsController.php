    ...
    public function action() {
        ...
        if (isset($_GET['download_auto_backup']) && !empty($_GET['download_auto_backup'])) {
            ...
            if (file_exists($file_path) && strpos($file_name, '.zip') !== false) {
                ...
                header('Location: /piura_noticias_php/admin/respaldos?msg=trash_ok');
                exit;
            }
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
            ...
            if ($_POST['action'] === 'full_zip') {
                ...
                try {
                    ...
                    if ($zip->open($zipFilePath, ZipArchive::CREATE | ZipArchive::OVERWRITE)) {
                        ...
                        try {
                            ...
                            foreach ($files as $name => $f) {
                                if (!$f->isDir()) {
                                    ...
                                    $isUpload = (strpos(str_replace(DIRECTORY_SEPARATOR, '/', $relativePath), 'uploads/') === 0);
                        ...
                        ob_end_clean();
                        header('Location: /piura_noticias_php/admin/respaldos?err=' . urlencode("No se pudo crear el archivo ZIP. Permisos denegados en la carpeta."));
                        exit;
                    }
                } catch (\Throwable $ex) {
                    ob_end_clean();
            ...
            if ($_POST['action'] === 'export') {
                ...
                if (empty($noticias)) {
                    header('Location: /piura_noticias_php/admin/respaldos?err=' . urlencode("No se encontraron noticias con los filtros seleccionados para exportar."));
                    exit;
                } else {
                    $backup_data = [
                        'metadata' => [
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                        ...
                        header('Location: /piura_noticias_php/admin/respaldos?err=' . urlencode("Error del sistema al generar el archivo JSON."));
                        exit;
                    }
                }
            }
            ...
            if ($_POST['action'] === 'import') {
                ...
                if (isset($_FILES['backup_file']) && $_FILES['backup_file']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if ($data && isset($data['metadata']['plataforma']) && $data['metadata']['plataforma'] === 'HTVPERU CMS' && isset($data['noticias']) && is_array($data['noticias'])) {
                        ...
                        header('Location: /piura_noticias_php/admin/respaldos?msg=import_ok&n=' . $imported . '&u=' . $updated . '&s=' . $skipped);
                        exit;
                    } else {
                        header('Location: /piura_noticias_php/admin/respaldos?err=' . urlencode("El archivo cargado no es un JSON válido o fue generado por otro sistema distinto a HTVPERU CMS."));
                        exit;
                    }
                } else {
                    header('Location: /piura_noticias_php/admin/respaldos?err=' . urlencode("Debes seleccionar un archivo JSON válido para importar."));
                    exit;
                }
            }
        }

        header('Location: /piura_noticias_php/admin/respaldos');
        exit;
    }
}
