...
<body>
    <div class="login-card">
        <div class="login-logo" style="display:flex; align-items:center; justify-content:center; gap:0.75rem;">
            <img src="<?= APP_BASE ?>/img/logo.webp" alt="Logo" style="height:48px; width:auto;">
        ...
        <form method="POST" action="<?= APP_BASE ?>/login.php">
            ...
            <a href="<?= APP_BASE ?>/login.php" style="display:block; margin-top:1.5rem; font-size:0.875rem; color:var(--text-muted); text-decoration:none;"><i class="ri-arrow-left-line"></i> Volver a Iniciar Sesión</a>
        </form>
        <?php else: ?>
        <form method="POST" action="<?= APP_BASE ?>/login.php">
            ...
            <div class="form-group">
                ...
                <a href="<?= APP_BASE ?>/login.php?recover=1" style="display:block; text-align:right; font-size:0.75rem; margin-top:0.5rem; color:var(--text-muted); text-decoration:none;">¿Olvidaste tu contraseña?</a>
            </div>
            <button type="submit" class="btn-submit">Ingresar Seguramente</button>
        </form>
        <?php endif; ?>
        
        <a href="<?= APP_BASE ?>/" style="display: block; margin-top: 1.5rem; font-size: 0.875rem; color: var(--primary-color);">Volver al sitio público</a>
    </div>
</body>
</html>
