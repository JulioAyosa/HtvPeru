        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:right; min-width: 220px;">
                    <a href="<?= APP_BASE ?>/admin/papelera/action?action_type=restore&type=<?=$t['type']?>&id=<?=$row['id']?>&csrf_token=<?=csrf_token()?>" class="btn-primary" style="background:#10b981; margin-right:4px;"><i class="ri-refresh-line"></i> Restaurar</a>
                    <a href="<?= APP_BASE ?>/admin/papelera/action?action_type=delete&type=<?=$t['type']?>&id=<?=$row['id']?>&csrf_token=<?=csrf_token()?>" class="btn-danger" onclick="return confirmDelete(event, '¿Seguro que deseas eliminar definitivamente esto?')"><i class="ri-delete-bin-fill"></i> Borrar</a>
                </td>
            </tr>
            <?php endforeach; ?>
    ...
    <div class="media-grid" id="grid-media">
        ...
        <div class="media-card filterable">
            <div class="media-preview">
                <img src="<?='<?= APP_BASE ?>/' . $f['path']?>" alt="<?=htmlspecialchars($f['name'])?>" onerror="this.outerHTML='<i class=\'ri-file-line\' style=\'font-size:3rem; color:#d1d5db;\'></i>'">
            </div>
            <div class="media-info">
                ...
                <div style="display:flex; gap:4px;">
                    <a href="<?= APP_BASE ?>/admin/papelera/action?action_type=restore&type=media&id=<?=urlencode($f['name'])?>&csrf_token=<?=csrf_token()?>" class="btn-primary" style="background:#10b981; padding:4px; width:100%; justify-content:center;" title="Restaurar"><i class="ri-refresh-line"></i></a>
                    <a href="<?= APP_BASE ?>/admin/papelera/action?action_type=delete&type=media&id=<?=urlencode($f['name'])?>&csrf_token=<?=csrf_token()?>" class="btn-danger" style="padding:4px; width:100%; justify-content:center;" title="Borrar Físicamente" onclick="return confirmDelete(event, '¿Eliminar definitivamente este medio?')"><i class="ri-delete-bin-fill"></i></a>
                </div>
            </div>
        </div>
        <?php endforeach; ?>
